...
const factory := preload("res://core/network/packets/factory.gd")
...
func _on_mouse_position_changed(normalized_pos: Vector2) -> void:
	var packet : packets.Packet = PacketFactory.new_player_msg(normalized_pos.x, normalized_pos.y)
	var err : Error = WS.send(packet)
	if err:
