...
func _ready():
	...
	var key = Global.printings_manager._make_key(metadata.uuid, metadata.setcode, metadata.number)
	var info = Global.printings_manager.get_card_info(key)
...
func _start_async_texture_load(path: String):
